...
class App extends Component {
  ...
  updateUserCandidate = async () => {
    ...
    await axios.get(`http://localhost:3000/users/${this.state.loggedInUserId}`, {
      headers: {
  ...
  setLoggedInUser = (userInfo, token) => {
    ...
    this.setState({loggedInUserId: userInfo.id, token: token, admin: userInfo.admin, userInfo: userInfo})
  }
  ...
  addVoterToMyVotersList = async (voterObj) => {
    ...
    await axios.get('http://localhost:3000/my-voters', {
      headers: {
        Authorization: this.state.token
      }
    }).then((myVoters) => {
  ...
  editVoters = async (voterObj) => {
    if (this.state.voter.eligible_voter_id) {
      ...
      }).then((response) => response.json()).then((newVoter) => {
  ...
  getinitialVoters = (votersArr) => {
    this.setState({myVoters: votersArr, filteredVoters: votersArr})
  }
  ...
  renderVoters = () => {
    return this.state.myVoters.filter((voter) => this.fullName(voter.eligible_voter.first_name, voter.eligible_voter.last_name).toLowerCase().includes(this.state.searchTerm.toLowerCase()))
  }
  ...
  filteredDropDown = (str) => {
    this.setState({isFiltered: str.toLowerCase()})
  }
  ...
  renderFiltered = (str) => {
    if (this.state.isFiltered === 'all') {
      ...
      return [...this.renderVoters()].sort((a, b) => a.eligible_voter.gender.localeCompare(b.eligible_voter.gender))
    }
  }
  ...
  votersNotHome = (voterObj) => {
    ...
    .then((response) => {
      ...
      } else if (response.contact_not_made_reason === 'No Such Address' || response.contact_not_made_reason === 'Moved') {
  ...
  render() {
    ...
    return (<div className='App'>
      ...
      <Switch>
        <Route path='/signup' render={(props) => <Signup {...props} setLoggedInUser={this.setLoggedInUser}/>}/>
        <Route path='/login' render={(props) => <Login {...props} setLoggedInUser={this.setLoggedInUser}/>}/>
        <Route path='/logout' render={(props) => <LogOut {...props}/>}/>
        <Route path='/dashboard/new-voter' render={(props) => (<NewVoter text='ADD' {...props} addVoterToMyVotersList={this.addVoterToMyVotersList}/>)}/>
        <Route path='/dashboard/edit-voter/:id' render={(props) => (<IndividualVoter text='EDIT' {...props} voter={voter} editVoters={this.editVoters}/>)}/>

        <Route path='/dashboard/my-voters' render={(props) => (<Voters {...props} canvas={false} loggedInUserId={loggedInUserId} userInfo={userInfo} searchVoter={this.searchVoter} filteredDropDown={this.filteredDropDown} token={token} voters={this.renderFiltered(isFiltered)} grabVoterDetail={this.grabVoterDetail} getinitialVoters={this.getinitialVoters}/>)}/>

        <Route exact='exact' path='/dashboard/in-person/:id' render={(props) => (<HomeScript {...props} votersNotHome={this.votersNotHome} token={token} voter={voter} votersHome={this.votersHome} setVoterNull={this.setVoterNull}/>)}/>

        <Route exact='exact' path='/dashboard/canvassing' render={(props) => (<Voters {...props} canvas={true} loggedInUserId={loggedInUserId} userInfo={userInfo} searchVoter={this.searchVoter} filteredDropDown={this.filteredDropDown} token={token} voters={this.renderFiltered(isFiltered)} grabVoterDetail={this.grabVoterDetail} getinitialVoters={this.getinitialVoters}/>)}/>

        <Route path='/dashboard/my-profile' render={(props) => (<Profile {...props} userInfo={JSON.parse(localStorage.userInfo)} token={localStorage.token} updateUserCandidate={this.updateUserCandidate}/>)}/>
